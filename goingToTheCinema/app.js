...
// card (price of the card), ticket (normal price of a ticket), perc (fraction of what he paid for the previous ticket) and returns the first n such that
...
const movie = (card, ticket, perc) => {
    ...
    while (Math.ceil(costA) <= Math.ceil(costB)) {
      ++trips;
      costA += ticket;
      costB += ticket * Math.pow(perc, trips);
